...
**ライブデモ**: https://script.google.com/macros/s/AKfycbz1NMadryd4_VxmKR9QiFZgZfJyUeF0TbTXEv1oE1M4ql5Dng2Xd_-x-1Ynw8FoqcxyzA/exec?action=dashboard
...
```
アクション: Webの内容を取得
URL: https://script.google.com/macros/s/AKfycbz1NMadryd4_VxmKR9QiFZgZfJyUeF0TbTXEv1oE1M4ql5Dng2Xd_-x-1Ynw8FoqcxyzA/exec
メソッド: POST
...
## ⚙️ Step 2: NFCオートメーション設定

### 2.1 オートメーション設定画面
![オートメーション設定](../images/オートメーション_設定.PNG)
...
### 2.2 NFCタグのスキャン設定
![NFCスキャン](../images/オートメーション_nfcスキャン.PNG)

**手順：**
1. 「スキャン」をタップ
2. NFCタグをiPhoneの上部（カメラ付近）にかざす
3. タグが認識されたら名前を設定（例：「太郎のポイントタグ」）
4. 「次へ」をタップ

### 2.3 ショートカット関連付け
1. 「ショートカットを実行」を選択
2. 作成したショートカットを選択
3. **重要**: 「実行前に尋ねる」をOFFにする
4. 「完了」をタップ
